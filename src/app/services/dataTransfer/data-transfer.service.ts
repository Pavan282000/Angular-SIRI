...
export class DataTransferService {
  ...
  postOrganizationDetails(organisation:any): Observable<any>{
    return this.http.post<any>(`http://127.0.0.1:5000/postOrganisation/${organisation.name}?sector=${organisation.sector}`, {
    });
  }
  ...
  postDimensionDetails(title:any,cut_off:any,weight:any,desc:any): Observable<any>{
    return this.http.post<any>(`http://127.0.0.1:5000/postDimension?dimensionTitle=${title}&weight=${weight}&cut_off=${cut_off}&description=${desc}`, {
    });
  }

  getModels(){
    return this.http.get(`http://127.0.0.1:5000/getModels`);
  }

  postModel(data:any){
    console.log(data)
      const config = { headers: new HttpHeaders().set('Content-Type', 'application/json')}
      return this.http.post(`http://127.0.0.1:5000/postModel`,data,config)
  }
  ...
  postDimension(Modelname:string,data:any){
    console.log(data)
    const config = { headers: new HttpHeaders().set('Content-Type', 'application/json')}
    return this.http.post(`http://127.0.0.1:5000/postDimension/?name=${Modelname}`,data,config)
  }

  getDimensions(model:any){
    return this.http.get(`http://127.0.0.1:5000/getDimensions?name=${model}`);
  }

  getDimensionsCount(model:any){
    return this.http.get(`http://127.0.0.1:5000/getDimensionsCount?name=${model}`);
  }

  getCompanies(){
    return this.http.get(`http://127.0.0.1:5000/getCompanies`)
  }
  ...
  postValue(orgName:any,modelName:any,data:any){
    const config = { headers: new HttpHeaders().set('Content-Type', 'application/json')}
    return this.http.post(`http://127.0.0.1:5000/postResults?orgname=${orgName}&modelName=${modelName}`,data,config)
  }

  postSurveyedModel(){
    console.log(this.selectedModel)
    console.log(this.Organisation)
    return this.http.post(`http://127.0.0.1:5000/postSurveyedResult?orgName=${this.Organisation}&sname=${this.selectedModel}`,{})
  }

  getResults(){
    console.log(this.selectedModel)
    console.log(this.Organisation)
    return this.http.get(`http://127.0.0.1:5000/getResults?orgName=${"Hero"}&sname=${"SIRI2"}`,{})
  }
